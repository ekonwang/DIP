...
class BackBone(nn.Module):
    def __init__(self, output_planes=512, name='resnet50', dilation=False, adaption=False):
        super().__init__()
        self.backbone = getattr(torchvision.models, name)(
                replace_stride_with_dilation=[False, False, dilation],
                pretrained=True)
        self.down_sampler = nn.AdaptiveAvgPool2d((1, 1))
        self.adaption = adaption
        if adaption:
            if name=='resnet50':
                self.adaptor = conv1x1(2048, out_planes=output_planes)
            elif name=='resnet34':
                self.adaptor = conv1x1(512, out_planes=output_planes)
            elif name=='resnet18':
                self.adaptor = conv1x1(512, out_planes=output_planes)
            else:
    ...
    def forward(self, x):
        # 保证参数 freeze。
        with torch.no_grad():
            features = self.backbone(x, return_feature=True)['feature_4']
            # (N, 512, H/8, W/8) 变为 (N, 512, 1, 1)。
            features = self.down_sampler(features)
        if self.adaption:
            adaptive_features = self.adaptor(features)
...
class SupervisedMAE(nn.Module):
    def __init__(self, img_size=384, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, relu_p=False, extract=None, adaption=False):
        super().__init__()
        ...
        if not extract:
            ...
            self.feature_extractor = BackBone(name=extract, adaption=adaption)
