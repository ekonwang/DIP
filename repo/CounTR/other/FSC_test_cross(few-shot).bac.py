...
from util.loss import PerturbationLoss, MincountLoss
...
def get_args_parser():
    ...
    parser.add_argument('--extract', type=str, default='resnet50', help='Use resnet backbone for examplar feature extraction')
    parser.add_argument('--adapt', action='store_true', help='Adapt features into test domain')
    ...
    parser.add_argument("--adapt_steps", type=int, default=10, help="step number for adaptation") 
...
def main(args):
    ...
    # define the model
    model = models_mae_cross.__dict__[args.model](norm_pix_loss=args.norm_pix_loss, relu_p=args.relu_p, extract=args.extract)
    ...
    if args.adapt:
        frozen_adaptor = copy.deepcopy(model.backbone.adaptor)
    ...
    for data_iter_step, (samples, gt_dots, boxes, pos, gt_map, im_name) in \
        ...
        RECTANGLE_SIZE_THRESHOLD = 10

        rectangle_count = 0
        small_rectangle_count = 0

        for rectangle in pos:
            rectangle_count += 1
            if rectangle_count > 3:
                break
            if rectangle[2] - rectangle[0] < RECTANGLE_SIZE_THRESHOLD and rectangle[3] - rectangle[1] < RECTANGLE_SIZE_THRESHOLD:
                small_rectangle_count += 1
                
                
        # Domain 自适应部分，可能效率较低。
        # Yikun:
        # 1. Min Count Loss 保证 examplar 附近的密度预测不少于 1。
        # 2. Perturbation Loss 保证密度分布接近高斯分布。
        if args.adapt:
            model.backbone.adaptor = frozen_adaptor
        # with torch.no_grad():
        #     latent = model.forward_encoder()
            optimizer = optim.Adam(model.backbone.adaptor.parameters(), lr=args.learning_rate)
            for i in range(args.adapt_steps):
                ...
                lCount = args.weight_mincount * MincountLoss(density_map / 60, pos)
                lPerturbation = args.weight_perturbation * PerturbationLoss(density_map / 60, pos, sigma=8)
        ...
        if small_rectangle_count >= 1:
            ...
            pred_cnt = 0
            assert len(r_images) > 0, "No region to predict"
        ...
        breakpoint()
        print(f'{data_iter_step}/{len(data_loader_test)}: pred_cnt: {pred_cnt},  gt_cnt: {gt_cnt},  error: {cnt_err},  AE: {cnt_err},  SE: {cnt_err ** 2}, id: {im_name.name}, s_cnt: {small_rectangle_count}')
        ...
        pred = density_map.unsqueeze(0) if small_rectangle_count < 1 else misc.make_grid(r_densities, h, w).unsqueeze(0)
        density_map = torch.cat((density_map, torch.zeros_like(density_map), torch.zeros_like(density_map))) * 5
